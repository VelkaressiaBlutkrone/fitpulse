...
[운영자] ──VPN/허용IP──▶ [Nginx] ──▶ [admin-web :3002]  (관리자 콘솔, V1.1)
                                  └──▶ [api /api/v1/admin/**]
```
...
| 관리자 콘솔 ★v2.0 | fitpulse-admin-web | 3002 | - | unless-stopped |
...
| `reports_pending` ★v2.0 | < 10건 | > 30건 또는 24h 초과 | 신고 처리 적체 |
| `routines_auto_blinded_today` ★v2.0 | 0~2건 | > 10건 | 어뷰징 또는 심사 기준 문제 |
| `admin_pii_unmask_count` ★v2.0 | 관리자당 < 20/일 | > 30/시간 | **내부자 대량 열람 의심** |
| `admin_login_failed_count` ★v2.0 | < 3/일 | > 10/시간 | 계정 탈취 시도 의심 |
| `approval_requests_pending` ★v2.0 | 0~2건 | 24h 초과 | 위험 작업 승인 지연 |
| `routine_like_rate_per_ip` ★v2.0 | - | 이상 급증 | 좋아요 어뷰징 |
...
| 디스크 > 85% | 🟡 P3 | Email | 당일 |
| **PII 마스킹 해제 30회/시간 초과** ★v2.0 | 🔴 **P1** | Slack + 테크리드 DM | 즉시 |
| 관리자 로그인 실패 10회/시간 ★v2.0 | 🟠 P2 | Slack | 30분 내 |
| 관리자 권한·역할 변경 발생 ★v2.0 | 🟠 P2 | Slack | 확인 |
| 감사 로그 기록 실패 ★v2.0 | 🔴 **P1** | Slack | 즉시 |
| 미처리 신고 24시간 초과 ★v2.0 | 🟡 P3 | Slack | 당일 |
| 자동 블라인드 10건/일 초과 ★v2.0 | 🟡 P3 | Slack | 당일 |
| AI 킬스위치 활성화 ★v2.0 | 🟠 P2 | Slack | 확인 |
| 점검 모드 4시간 초과 지속 ★v2.0 | 🟠 P2 | Slack | 확인 |
...
| 일 1회 | Grafana 대시보드 육안 확인, 백업 성공 여부, ERROR 로그 검토, ★v2.0 **미처리 신고 큐 확인** |
| 주 1회 | 디스크 사용량, 슬로우 쿼리 리뷰, AI 비용 추이 |
| 월 1회 | 백업 복원 테스트, 보안 패치 확인, 미사용 이미지 정리, ★v2.0 **감사 로그 리뷰(PII 열람·민감 작업 이력)** |
| 분기 1회 | 본 매뉴얼 갱신, 장애 대응 훈련, 용량 산정 재검토, ★v2.0 **관리자 계정 棚卸(재직·역할 적정성 검토)** |
...
| 랭킹 스냅샷 ★v2.0 | 매시 정각 | Redis ZSET 상위 100건 영속화 | `routine_ranking_snapshots` 신규 행 |
| 좋아요 집계 보정 ★v2.0 | 매일 04:30 | `routine_stats.like_count` 실측 재집계 (D-13) | 로그 `StatsReconcileScheduler` |
| 제재 만료 처리 ★v2.0 | 매시 | `ends_at` 경과 제재 자동 해제 | `moderation_actions` |
| 승인 요청 만료 ★v2.0 | 매시 | 24시간 경과 요청 EXPIRED 전환 | `admin_approval_requests` |
| 감사 로그 파티션 점검 ★v2.0 | 월 1회 | 1천만 건 초과 시 월 파티션 추가 | 수동 확인 |
...
| 4 | `JWT_SECRET` 교체 검토 (교체 시 전체 로그아웃 발생) / ★v2.0 관리자 침해 시 `ADMIN_JWT_SECRET`도 함께 교체 |
| 5 | 접근 로그 보존 후 원인 분석 |
| 6 | 개인정보 유출 정황 시 PO 즉시 보고 → 관련 법령상 신고 의무 검토 |


### 4.10 Runbook: 관리자 계정 탈취 의심 (P1) ★ v2.0

| 단계 | 조치 | 명령 / 화면 |
|------|------|-------------|
| 1 | 해당 관리자 전 세션 즉시 폐기 | `UPDATE admin_sessions SET revoked_at=NOW() WHERE admin_id=?;` |
| 2 | 계정 비활성화 | 콘솔 [관리자 계정] → DISABLED (또는 SQL `UPDATE admins SET status='DISABLED'`) |
| 3 | 해당 계정의 최근 행위 전수 확인 | `SELECT * FROM admin_audit_logs WHERE admin_id=? AND created_at > NOW()-INTERVAL 7 DAY ORDER BY created_at;` |
| 4 | 피해 범위 산정 | PII 열람·제재·설정 변경 이력 집계 |
| 5 | 변경 사항 원복 | 감사 로그의 `before_value`를 근거로 수동 복구 |
| 6 | TOTP 재등록 + 비밀번호 재설정 | 본인 확인 후 SUPER_ADMIN이 초기화 |
| 7 | 개인정보 유출 정황 시 | **PO 즉시 보고 → 법령상 신고 의무 검토** |

**예상 복구 시간**: 세션 차단 5분 / 피해 산정 30분~2시간

### 4.11 Runbook: 내부자 대량 개인정보 열람 탐지 (P1) ★ v2.0

알림: `admin_pii_unmask_count > 30/시간`

| 단계 | 조치 |
|------|------|
| 1 | 해당 관리자·시간대 열람 이력 확인: `SELECT target_id, reason, created_at FROM admin_audit_logs WHERE action='USER_PII_UNMASK' AND admin_id=? AND created_at > NOW()-INTERVAL 2 HOUR;` |
| 2 | 사유가 업무와 정합한지 검토 (문의 번호가 실제 존재하는가) |
| 3 | **정당한 업무**: 임계치 일시 상향 검토 + 사유 기록 |
| 4 | **설명 불가**: 계정 즉시 비활성화 + 세션 폐기 + PO 보고 |
| 5 | 열람 대상 회원 목록 확보 (유출 시 통지 대상) |

> 이 절차는 특정인을 의심하기 위한 것이 아니라, 임계치가 울렸을 때 **일관되게 확인하는 방법**을 정해둔 것입니다. 대부분은 정당한 업무이며, 그 경우에도 확인 결과를 기록으로 남깁니다.

### 4.12 Runbook: 감사 로그 기록 실패 (P1) ★ v2.0

감사 기록이 실패하면 관리자 작업 자체가 롤백되도록 설계되어 있습니다. 즉 **관리자 기능이 전면 중단된 상태**입니다.

| 단계 | 조치 |
|------|------|
| 1 | `docker compose logs api \| grep -i "audit"` 로 원인 확인 |
| 2 | 권한 문제인지 확인: `SHOW GRANTS FOR 'fitpulse'@'%';` → INSERT 권한 존재 여부 |
| 3 | INSERT 권한이 없으면 재부여 (배포 가이드 3.8 재실행, **UPDATE/DELETE는 부여 금지**) |
| 4 | 디스크·파티션 문제면 4.8 수행 |
| 5 | 복구 전까지 관리자 작업 중단 유지 (우회 금지) |

> ⚠️ **긴급하다고 감사 기록을 우회하거나 비활성화하지 마세요.** 기록 없는 관리 작업은 사후에 무엇이 일어났는지 확인할 수 없게 만듭니다. 복구 후 정상 경로로 수행합니다.

### 4.13 Runbook: 위험 콘텐츠 긴급 차단 (P2) ★ v2.0

신고가 급증하거나 명백히 위험한 공개 루틴이 확인된 경우.

| 단계 | 조치 |
|------|------|
| 1 | 콘솔 [콘텐츠 > 공개 심사] → 해당 루틴 [숨김 처리] (즉시 목록 제외) |
| 2 | 동일 작성자의 다른 공개 루틴 일괄 점검 |
| 3 | 필요 시 공개 권한 정지 집행 |
| 4 | 신고자에게 처리 결과 통지 |
| 5 | 심사 기준에 반영할 사항이면 운영 기준 문서 갱신 |

**콘솔 접근이 불가한 긴급 상황의 임시 조치**
```sql
UPDATE routines SET review_status='BLINDED',
  review_note='긴급 차단 (사유 기재)' WHERE id = ?;
```
> 이 SQL은 **감사 로그를 남기지 않으므로** 최후 수단입니다. 수행 시 반드시 Slack에 시각·대상·사유·수행자를 남기고, 콘솔 복구 후 정식 절차로 재기록합니다.

### 4.14 Runbook: 신고 처리 적체 (P3) ★ v2.0

| 단계 | 조치 |
|------|------|
| 1 | `reports_pending` 및 최고령 신고 경과 시간 확인 |
| 2 | 30건 초과 시 임계치 임시 하향(자동 블라인드 5 → 3)으로 1차 방어 |
| 3 | 유형별 분포 확인 — 특정 유형 급증이면 어뷰징 가능성 검토 |
| 4 | 반복 신고 대상은 일괄 처리 |
| 5 | 지속되면 운영 인력 배정을 PO와 협의 |

### 4.15 Runbook: 랭킹 이상 (P3) ★ v2.0

| 증상 | 확인 | 조치 |
|------|------|------|
| 랭킹이 갱신되지 않음 | `RankingSnapshotScheduler` 로그 | 스케줄러 재기동 |
| 순위가 비상식적 | 동일 IP 다계정 좋아요 여부 | 어뷰징 계정 정지 + 점수 재계산 |
| `calculatedAt`이 과거 | Redis 상태 | 4.5 Redis Runbook 수행 (현재는 스냅샷 폴백 중) |
| 카운트 불일치 | `routine_stats` vs `routine_likes` 실측 | 좋아요 집계 보정 배치 수동 실행 |

### 4.16 Runbook: 점검 모드 운영 ★ v2.0

**시작**
1. 최소 3일 전 공지 등록 (긴급 시 즉시)
2. 콘솔 [설정 > 점검 모드] ON + 안내 문구 + 예상 종료 시각
3. 사용자 쓰기 API 503 확인, 읽기 정상 확인

**종료**
1. 점검 모드 OFF
2. 오프라인 큐 동기화 유입 확인 (`offline_ops_pending` 감소)
3. 5xx 오류율 5분 관찰
4. 공지 종료 처리

> 점검 모드가 4시간을 넘기면 알림이 발생합니다. 장기 점검이 필요하면 사용자 공지를 갱신하세요.
...
| 관리자 계정·권한 ★v2.0 | SUPER_ADMIN | (등록 필요) | 24시간 (P1) |
| 신고·제재 심사 ★v2.0 | 운영 담당 | (등록 필요) | 업무시간 |
...
| 개인정보 유출 정황 ★v2.0 | 필수 | 앱 내 + 이메일 + 법정 통지 | 확인 즉시 (법령 기한 준수) |
| 커뮤니티 정책 변경 ★v2.0 | 필수 | 앱 내 공지 | 시행 7일 전 |
...
## 8. 관리자 운영 정기 점검표 ★ v2.0

### 8.1 일일 (운영 담당)

| # | 항목 | 기준 |
|---|------|------|
| 1 | 미처리 신고 확인 | 24시간 초과 0건 |
| 2 | 자동 블라인드 발생 건 검토 | 전건 확정/해제 판정 |
| 3 | 공개 심사 대기 처리 | 48시간 이내 |
| 4 | 승인 대기 요청 확인 | 24시간 내 판정 |

### 8.2 월간 (테크리드)

| # | 항목 | 확인 방법 |
|---|------|-----------|
| 1 | PII 열람 이력 리뷰 | `action='USER_PII_UNMASK'` 사유 적정성 |
| 2 | 민감 작업 이력 리뷰 | 영구정지·삭제·설정변경 전건 |
| 3 | 권한 변경 이력 | `ADMIN_ROLE_MANAGE` 발생 건 |
| 4 | 실패 기록(DENIED) 분석 | 권한 오설정 또는 부적절 접근 시도 |
| 5 | 감사 로그 건수·용량 | 파티션 추가 필요 여부 |

### 8.3 분기 (SUPER_ADMIN)

| # | 항목 |
|---|------|
| 1 | 관리자 계정 棚卸 — 재직 여부, 역할 적정성, 미사용 계정 비활성화 |
| 2 | 권한 매트릭스 재검토 — 과도 부여 여부 |
| 3 | IP 허용 목록 갱신 |
| 4 | 백업 코드 재발급 필요 계정 확인 |
| 5 | 본 매뉴얼 및 관리자 매뉴얼 갱신 |
...
| v2.0 | 2026-08-13 | 테크리드 | 커뮤니티·관리자 운영 반영 (Runbook 4.10~4.16, 관리자 지표·알림, 정기 점검표) |
