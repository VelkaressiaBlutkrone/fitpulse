...
| 버전 | v2.0 |
| 작성일 | 2026-08-13 |
| 관련 요구사항 | NFR-301~305, NFR-201~205, NFR-101~114 |
| 변경 | v2.0 — 커뮤니티(CR-013)·관리자(CR-014) 아키텍처 반영, ADR-007~010 추가 |
...
| AP-9 | **관리자 평면 분리** ★v2.0 | 관리자는 별도 계정 테이블·별도 토큰·별도 경로(`/admin/**`)·별도 세션 정책. 회원 계정 탈취가 관리 권한 획득으로 이어지지 않음 |
| AP-10 | **모든 관리 행위는 추적 가능** ★v2.0 | 쓰기뿐 아니라 개인정보 조회까지 감사 로그. 로그는 수정·삭제 불가 |
| AP-11 | **되돌리기 어려운 작업은 2인** ★v2.0 | 영구정지·하드 삭제·설정 변경은 요청/승인 분리 |
| AP-12 | **런타임 설정으로 즉시 대응** ★v2.0 | 점검 모드·AI 차단·한도 조정을 재배포 없이 전환 (`system_settings`) |
...
│  ├── 인증: 이메일+비밀번호 → TOTP 2FA → 관리자 세션(30분)                   │
│  ├── 화면: 회원 / 콘텐츠 / 신고·제재 / AI 운영 / 통계 / 감사 로그 / 설정     │
│  └── 배포: 정적 빌드 → Nginx, 내부망 또는 IP 허용 목록                       │
└──────────────────────────────────┬─────────────────────────────────────────┘
                                   │ HTTPS  /api/v1/admin/**  (adminToken)
┌──────────────────────────────────▼─────────────────────────────────────────┐
...
├── community/       ★v2.0 공개 루틴·좋아요·랭킹·신고
├── moderation/      ★v2.0 신고 심사·제재·이의제기
├── admin/           ★v2.0 관리자 인증·RBAC·감사·운영 도구
└── shared/          공통 (이벤트, 예외, 유틸, 보안, 설정)
...
모듈 간:
  ...
  community ──[RoutineLiked / RoutineCopied 이벤트]──▶ community(stats, Redis ZSET)
  community ──[RoutineReported 이벤트]──▶ moderation (임계치 도달 시 자동 블라인드)
  moderation ──[SanctionApplied 이벤트]──▶ user (세션 폐기), community (콘텐츠 숨김)
  admin ──[AdminActionPerformed 이벤트]──▶ admin (감사 로그 기록)
  ※ 다른 모듈의 Entity/Repository 직접 참조 금지
  ※ admin 모듈은 다른 모듈의 application 계층 인터페이스만 호출하며,
    도메인 규칙을 우회하는 직접 DB 조작을 하지 않는다
```
...
| `RoutineLiked` ★v2.0 | community | community | `routine_stats.like_count` 갱신, Redis ZSET 점수 증가 | 커밋 후 비동기 |
| `RoutineCopied` ★v2.0 | routine | community | copy_count 갱신, 점수 증가 | 커밋 후 비동기 |
| `RoutineReported` ★v2.0 | community | moderation | report_count 증가, 임계치 도달 시 자동 블라인드 | 커밋 후 비동기 |
| `SanctionApplied` ★v2.0 | moderation | user, community | 세션 폐기, 콘텐츠 노출 중단, 사용자 통지 | 커밋 후 비동기 |
| `AdminActionPerformed` ★v2.0 | admin | admin | `admin_audit_logs` 기록 | **동기(같은 트랜잭션)** |
...
관리자                     API                         DB / TOTP
  ...
  │                         │ ③ BCrypt 검증               │
  │                         │    실패 5회 → locked_until  │
  │◀── mfaToken(5분) ───────│    (기능 API 호출 불가)      │
  │ ④ TOTP 6자리            │                            │
  │────────────────────────▶│ ⑤ TOTP 검증(±30초 윈도우)  │
  │                         │ ⑥ admin_sessions INSERT    │
  │                         │    (ip, ua, absolute 8h)   │
  │◀── adminToken(30분) ────│    동시 세션 2개 초과 시 최고령 폐기
```

### 5-A.2 요청 처리 파이프라인

```
요청 → AdminAuthFilter        : adminToken 검증, 발급자(admins) 확인
     → SessionPolicyFilter    : Idle 30분 / 절대 8시간 / mfa_verified 확인
     → IpAllowListFilter      : allowed_ip_cidrs 검사
     → @RequirePermission     : 메서드 선언 권한 코드 보유 확인 (미보유 403)
     → ReasonRequiredCheck    : 민감 작업이면 X-Admin-Reason 필수
     → ApprovalGate           : is_sensitive & 2인 대상이면 승인 요청 생성(202)
     → UseCase 실행
     → AuditAspect            : 결과(SUCCESS/DENIED/FAILED) + before/after 기록
```

> 감사 기록은 **UseCase와 같은 트랜잭션**에서 수행합니다. 작업은 성공했는데 로그가 남지 않는 상태를 허용하지 않기 위해서입니다. 감사 기록 실패 시 작업 전체가 롤백됩니다.

### 5-A.3 관리자 위협 모델과 통제

| 위협 | 통제 |
|------|------|
| 관리자 계정 탈취 | 별도 계정 체계 + TOTP 2FA + IP 허용 목록 + 짧은 세션 |
| 내부자 대량 개인정보 열람 | 기본 마스킹 + 해제 시 사유 필수 + 시간당 30회 제한 + 임계 초과 시 Slack 알림 |
| 권한 상승 | 역할·권한 변경은 SUPER_ADMIN 전용, 변경 자체가 감사 대상 |
| 흔적 삭제 | 감사 로그 DB 권한에서 UPDATE/DELETE 회수, API 미제공 |
| 단독 파괴 행위 | 영구정지·하드 삭제·설정 변경은 2인 승인 (요청자≠승인자) |
| 퇴사자 접근 유지 | 계정 비활성화 시 전 세션 즉시 폐기, 분기 1회 계정 棚卸 |
| 관리자 콘솔 외부 노출 | 운영 환경은 내부망/VPN 또는 Nginx IP 허용 목록 |
| 회원 데이터 임의 변조 | 운동·건강 데이터 수정 API 자체를 제공하지 않음 |

### 5-A.4 관리자 웹 배포 아키텍처 (V1.1)

```
[운영자] ──VPN/허용IP──▶ [Nginx] ──┬── /            → admin-web (React 정적 빌드)
                                    └── /api/v1/admin → fitpulse-api

- 관리자 웹은 별도 컨테이너(admin-web)에서 정적 파일 서빙
- 사용자 앱과 동일 API 서버를 사용하되 경로·인증 체계가 분리됨
- CSP·X-Frame-Options DENY·HSTS 적용
- 토큰은 메모리 보관, 새로고침 시 재인증 (localStorage 저장 금지)
```
...
| 랭킹 | Redis Sorted Set + 매시 스냅샷 | 규모 증가 시 별도 랭킹 워커 분리 |
| 관리자 콘솔 | 정적 빌드 1컨테이너 | CDN 배포 또는 복제 |
...
| ADR-007 ★v2.0 | 관리자 계정을 users에 통합 vs 별도 테이블 | **별도 `admins` 테이블** | 회원 계정 탈취가 관리 권한으로 이어지지 않도록 인증 평면 분리. `users.role`은 하위 호환용으로만 유지 |
| ADR-008 ★v2.0 | 감사 로그 저장 방식 | **DB 테이블 + 권한 수준 불변성** | 파일 로그는 위변조·유실 대응이 약함. INSERT/SELECT만 GRANT하여 애플리케이션 경로로도 수정 불가 |
| ADR-009 ★v2.0 | 랭킹 저장소 | **Redis ZSET + 매시 스냅샷 영속화** | 실시간 점수 갱신은 RDB 부하가 큼. Redis 유실 위험은 스냅샷 폴백으로 보완 |
| ADR-010 ★v2.0 | 공개 루틴 심사 방식 | **사전 심사 + 신고 누적 자동 블라인드** | 2인 운영 체제에서 전건 실시간 심사는 불가. 사전 심사로 최소 품질을 거르고 사후 신고로 보완 |
...
```markdown
# ADR-007: 관리자 계정을 별도 테이블로 분리
## 상태: 수락됨
## 컨텍스트
초기 ERD는 `users.role = USER/ADMIN`으로 관리자를 표현했다. 구현이 단순하지만,
회원 계정 하나가 탈취되면 곧바로 전체 회원 데이터에 접근할 수 있는 구조가 된다.
또한 관리자에게 필요한 2FA·IP 제한·짧은 세션·감사 정책을 일반 회원 정책과
같은 테이블에서 관리하면 정책이 서로 오염된다.
## 결정
`admins` 테이블과 `admin_sessions`를 신설하고, 관리자 인증·인가·세션 정책을
사용자 평면과 완전히 분리한다. 관리자 API는 `/api/v1/admin/**` 별도 경로를 쓰고,
사용자 토큰으로는 접근할 수 없다. `users.role`은 삭제하지 않고 유지하되
인가 판단에는 사용하지 않는다(하위 호환 원칙 AP-8).
## 대안
- users.role 유지: 구현 단순하나 위 위험 해소 불가
- 외부 IdP(SSO) 도입: 보안상 우수하나 2인 팀·MVP 예산에 과함 (V2 재검토)
## 결과
(+) 회원 계정 탈취와 관리 권한 획득이 분리됨
(+) 관리자 전용 보안 정책(2FA·IP·30분 세션)을 독립적으로 적용
(−) 계정 체계가 둘로 늘어 초대·비밀번호 재설정 흐름을 별도 구현해야 함
(−) 관리자가 일반 사용자로도 활동하려면 계정 2개 필요 (의도된 제약)
## 날짜: 2026-08-13
```
