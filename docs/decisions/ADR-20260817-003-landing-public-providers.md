...
2. **이메일 발송 — 소유자 선택 필요**: 두 선택지의 맞교환이 명확해졌다.
...
- [ ] **이메일 공급자 선택**: 권고안 2의 (a) SES 서울 / (b) Resend 중 선택
- [ ] **D1 생성 위치 결정**: `apac` 등 힌트 선택. 생성 후 변경 불가
- [ ] 랜딩 공개 시 사용할 호스팅과 그 데이터 위치·로그 보존
- [ ] Brevo를 후보에서 제외할지, 추가 조사할지 결정
- [ ] 선정 공급자의 수탁 계약(DPA) 조건 확인
