...
- **D1 Free 플랜 읽기 전용 전환 대응** (WF-01 조사에서 추가된 범위). 계정이 Workers Free로 확인되어 D1 일일 한도는 5,000,000 행 읽기 / 100,000 행 쓰기이고, 초과 시 다음 날까지 데이터베이스가 읽기 전용이 된다. 읽기 전용 상태에서는 대기자 등록·설문·이벤트 기록뿐 아니라 **삭제 요청 처리도 실패**하므로 비용 문제가 아니라 가용성·의무 이행 문제다. 다음을 다룬다.
  - 정상 방문 1회당 D1 행 쓰기 수 산정과 일 100,000 행 대비 여유 계산
  - 한도 접근 감지 수단. Free 플랜은 Logpush를 쓸 수 없으므로 Workers Logs 3일 보존 범위 또는 대시보드 수동 점검에 의존한다
  - 읽기 전용 전환 시 사용자에게 표시할 상태와 삭제 요청 대체 경로
  - 한도 소진을 노린 남용 요청 차단 — WF-02의 방어 수단과 연계
