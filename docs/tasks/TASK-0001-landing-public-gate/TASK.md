...
- 데이터 위험: **프로덕션 D1이 존재하지 않는다.** 2026-08-17 `wrangler d1 info site-creator-d1` 실행 결과 계정에 해당 데이터베이스가 없음이 확인되었다. 지금까지의 검증은 전부 로컬 D1에서만 이뤄졌고, 원격 D1의 데이터 위치·백업·처리 국가는 검증된 적이 없다. 공개 전 D1 생성이 선행 작업으로 추가되어야 한다.
- 데이터 위험: D1 위치 힌트(`wnam`/`enam`/`weur`/`eeur`/`apac`/`oc`)는 **생성 시에만 지정할 수 있고 이후 변경이 불가능**하며, 지정해도 배치 위치가 보장되지 않는다. 개인정보 안내에 특정 국가를 확정적으로 쓸 수 없다.
- 문서 신뢰성 위험: 실행 계획 88행이 "Cloudflare D1의 1차 이벤트"로 체크되어 있으나 그 시점에 원격 D1은 존재하지 않았다. WF-06에서 이 항목을 미확인으로 되돌려 재판정해야 한다.
