...
### 2026-08-17 — WF-03 구현 (이메일 확인 흐름, 로컬 완결 범위)

배포 보류 지시에 따라 **로컬에서 검증 가능한 범위까지** 구현했다. AWS 프로덕션 액세스 승인은 실제 발송 검증에만 필요하므로 구현을 막지 않았다.

**구현**

- 확인 토큰: 32바이트 랜덤, **해시만 저장**(기존 `management_token_hash` 패턴 재사용), 만료 14일
- `landing/drizzle/0003_omniscient_la_nuit.sql` — `confirmation_token_hash`, `confirmation_expires_at` **추가만**. 기존 컬럼·마이그레이션 무변경
- 확인 링크 처리 → `verified_at` 설정, 토큰 해시 삭제로 재사용 차단
- `app/lib/email.ts` — 발송 어댑터 경계. 공급자 종속 코드를 이 안에만 둔다
- `app/confirm/page.tsx` — 확인 결과 서버 렌더. **토큰이 최종 주소에 남지 않는다**
- `/api/waitlist/summary` — 확인된 항목만 계수

**설계 결정 3건**

1. **`waitlist_submit`의 의미를 바꾸지 않았다.** 저장 성공 시점을 계속 뜻하며, 확인 완료는 새 이벤트 `waitlist_confirm`으로 기록한다. 기존 기록과의 비교 가능성을 지키기 위함이다
2. **발송 실패가 등록을 막지 않는다.** 미확인으로 남고 14일 뒤 정리된다. 성공·실패 모두 같은 응답이라 이메일 존재 여부가 드러나지 않는다
3. **확인 결과를 서버 컴포넌트로 렌더한다.** 처음에는 클라이언트 effect로 처리했으나 `react-hooks/set-state-in-effect` lint 오류가 났고, 서버 렌더가 토큰 유출 방지에도 낫다

**중복 등록 처리**: 새 토큰을 발급하지 않는다. 기존 확인 링크가 그대로 유효하다.

**검증**: `npm test` 20개 통과(신규 5개), lint 오류 0, audit 0건.

**Not Run 2건**: 실제 SES 연동·발송 검증(AWS 승인 대기), 실기기 뷰포트 QA(배포 보류).

### 2026-08-17 — WF-04 구현 (개인정보 안내 정합화)

WF-02·WF-03·WF-07·WF-09에서 확인한 사실을 안내 문구에 반영했다. **불리한 사실을 감추지 않는 것**이 이 작업의 핵심이다.

**동의 버전을 v3로 올렸다.** 안내 내용이 실질적으로 바뀌었는데 같은 버전을 유지하면 "무엇에 동의했는지" 추적이 무의미해진다. 실사용자가 0명이라 마이그레이션 부담이 없는 지금이 적기다. 테스트의 과거 데이터 삽입은 `prevalidation-v2`로 남겨 버전 혼재 상황을 반영했다.

**안내에 새로 넣은 사실 6건**

| 사실 | 근거 |
|---|---|
| 저장 국가를 특정할 수 없음 | WF-07 — 배포 환경이 레지던시 미지원 |
| 삭제 후 플랫폼 내부 **최대 30일** 잔존 | WF-07 |
| 예약 작업 미지원으로 정리가 지연될 수 있음 | WF-07·WF-09 |
| 수탁 구조 2단(OpenAI → 재위탁) | WF-07 |
| 재위탁 사업자가 페이지에 보안·안전 분류기 실행 | WF-07 |
| Cloudflare(Turnstile)·AWS(발송 예정) 수탁 추가 | WF-01·WF-02 |

**정정한 것**

- "일일 정리 작업으로 삭제합니다" → 실제 두 경로와 한계를 설명하는 문구로 교체
- "이메일 확인·발송 자동화가 연결되지 않았습니다" → 확인 흐름은 구현됐고 발송 공급자만 미연결임을 구분
- 보존 기간 표기를 "12개월"에서 코드 상수와 같은 "365일"로 통일

**회귀 방지**: 안내가 명시한 보존 기간이 `landing-storage.ts` 상수와 일치하는지 테스트로 강제한다. 상수를 바꾸면 테스트가 실패하므로 안내와 코드가 조용히 어긋날 수 없다.
