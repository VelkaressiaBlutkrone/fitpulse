...
확인되지 않은 이메일은 14일, 그 밖의 대기자·설문·이벤트 데이터는 최대 365일 보관한다. 배포 환경이 예약 작업을 지원하지 않으므로 만료 삭제는 요청 시점 정리와 외부 스케줄러 두 경로로 실행한다. 같은 브라우저에서는 등록 직후 이메일과 연결 설문을 직접 삭제할 수 있다. 저장소에서 지운 뒤에도 배포 환경 내부에 최대 30일 남을 수 있다. 운영 절차는 [데이터 삭제·보존 런북](../docs/runbooks/fitpulse-landing-data-deletion-retention.md)을 따른다.
